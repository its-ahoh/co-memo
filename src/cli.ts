#!/usr/bin/env node
import { configuration, configure, remember, change } from './service.js';
...
app
  ...
  .option('--query <text>', 'Case-insensitive literal text filter')
  .action((opts: { deleted?: boolean; query?: string }) =>
    print(
      using((store, root) => {
        ...
        reportExit(report);
        return store
          .list(project.id, opts.deleted)
          .filter((m) => !opts.query || m.content.toLowerCase().includes(opts.query.toLowerCase()));
      }),
    ),
  );
app
  ...
  .action(() => {
    process.stdout.write(
      using((store, root) => {
        const report = sync(store);
        reportExit(report);
        return context(store, store.project(root).id);
      }),
    );
  });
app
  ...
  .action((opts: { agent: string; event: string }) => {
    ...
    const result = using((store, root) => {
      ...
      const warning =
        report.errors.length || report.conflicts.length
          ...
          : '';
      return { ...report, context: context(store, project.id) + warning };
    });
